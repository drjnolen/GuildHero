...
async def preflight_airdrop(sender_address: str, recipient_count: int, amount: int, coin_type: str,
                            *, total_amount: int | None = None, batch_count: int | None = None) -> dict:
    if total_amount is None:
        requirements = build_airdrop_balance_requirements(recipient_count, amount, coin_type, SUI_GAS_BUDGET)
    else:
        requirements = {
            "required_sui_balance": SUI_GAS_BUDGET * batch_count + (total_amount if coin_type == DEFAULT_SUI_COIN_TYPE else 0),
            "required_token_balance": 0 if coin_type == DEFAULT_SUI_COIN_TYPE else total_amount,
        }
...
async def _report_airdrop(message, run):
    ...
    for batch in run["batches"]:
        ...
        if batch["status"] == "sent":
            total_sent += sum(int(item["amount"]) for item in batch["recipients"])
    lines.extend([
        f"✅ Sent: {counts['sent']} | ⏭️ Skipped: {len(run['skipped'])} | ❌ Failed: {counts['failed']}",
        f"⏳ Unconfirmed: {counts['unknown'] + counts['submitted']} | Not sent: {counts['not_sent']}",
        f"Confirmed total: {format_token_amount(total_sent, run['decimals'])} {html.escape(run['symbol'])}",
    ])
    for index, batch in enumerate(run["batches"], 1):
        lines.append(f"\n<b>Batch {index}: {batch['status'].replace('_', ' ')}</b>")
        if batch.get("digest"):
            lines.append(f"Transaction: <code>{html.escape(batch['digest'])}</code>")
        if batch.get("error"):
            lines.append(html.escape(batch["error"][:250]))
        for item in batch["recipients"]:
            lines.append(f"#{item['rank']} @{html.escape(item['username'])}: "
...
async def airdrop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lock.locked():
        await update.message.reply_text("An airdrop is already running for this group. Please wait.")
        return
    async with lock:
        await _airdrop_command_locked(update, context)


async def _airdrop_command_locked(update, context):
    chat_id = update.effective_chat.id
    service = get_sui_service(SUI_GRPC_URL, SUI_GRPC_HEADERS)
    latest_key = f"airdrop_latest:{chat_id}"
    if context.args and context.args[0].lower() == "status":
        if len(context.args) > 2 or (len(context.args) == 2 and not context.args[1].isdigit()):
            await update.message.reply_text("Use /airdrop status or /airdrop status <id>.")
            return
        run_id = context.args[1] if len(context.args) == 2 else await asyncio.to_thread(db.get, latest_key)
        key = f"airdrop_run:{chat_id}:{run_id}"
        run = await asyncio.to_thread(db.get, key)
        if not run:
            await update.message.reply_text("No saved airdrop found for this group.")
            return
    ...
    if not context.args:
        await update.message.reply_text(
            "Reply to a /score leaderboard with:\n"
            "/airdrop 10 10000 — 10,000 each to ranks 1–10\n"
            "/airdrop 1-5:30k 6-10:15k — different rewards by rank\n"
            "/airdrop 1:50k 2-5:30k 6-10:15k\n"
            "k = thousand; m = million. Tiers must be consecutive from rank 1. "
            "Missing wallets are skipped without changing ranks.\n"
            "/airdrop status — check the latest saved airdrop.")
        return
    if not update.message.reply_to_message:
        await update.message.reply_text("Please reply to a /score leaderboard to send an airdrop.")
        return
    leaderboard = _get_leaderboard_messages(context).get((chat_id, update.message.reply_to_message.message_id))
    if not leaderboard:
        await update.message.reply_text("The replied message is not a recognized /score leaderboard.")
        return
    run_id = str(update.message.message_id)
    key = f"airdrop_run:{chat_id}:{run_id}"
    existing = await asyncio.to_thread(db.get, key)
    if existing:
        await reconcile_batches(service, db, key, existing)
        await _report_airdrop(update.message, existing)
        return
    previous_id = await asyncio.to_thread(db.get, latest_key)
    previous_key = f"airdrop_run:{chat_id}:{previous_id}"
    previous = await asyncio.to_thread(db.get, previous_key) if previous_id else None
    if previous:
        await reconcile_batches(service, db, previous_key, previous)
        if any(batch["status"] in ("unknown", "submitted") for batch in previous["batches"]):
            await update.message.reply_text(
    ...
    try:
        ...
        return
    recipients, skipped = [], []
    for rank, entry in enumerate(leaderboard[:tiers[-1][1]], 1):
        username, _, _, user_id = entry
        amount = next(amount for start, end, amount in tiers if start <= rank <= end)
        wallet_data = await asyncio.to_thread(get_wallet, chat_id, int(user_id))
        wallet = normalize_wallet_address(wallet_data.get('wallet_address', '')) if wallet_data else None
        if not wallet:
            skipped.append({"rank": rank, "username": username, "reason": "No valid registered wallet"})
            continue
        recipients.append({"rank": rank, "username": username, "user_id": str(user_id),
                           "wallet": wallet, "amount": str(amount)})
    if not recipients:
        await update.message.reply_text("None of the selected ranks have valid registered wallets.")
        return
    try:
        ...
        total = sum(int(item['amount']) for item in recipients)
        await preflight_airdrop(sender['wallet_address'], len(recipients), 0, coin_type,
                                total_amount=total, batch_count=len(batches))
    except Exception as exc:
        await update.message.reply_text(f"Airdrop preflight failed: {exc}")
        return
    run = {"id": run_id, "coin_type": coin_type, "decimals": config['decimals'],
           "symbol": config['symbol'], "sender": sender['wallet_address'],
           "leaderboard_message_id": update.message.reply_to_message.message_id,
           "tiers": tiers, "skipped": skipped,
           "batches": [{"recipients": batch, "status": "not_sent"} for batch in batches]}
    await asyncio.to_thread(db.__setitem__, key, run)
    await asyncio.to_thread(db.__setitem__, latest_key, run_id)
    await update.message.reply_text(
        f"🪂 Sending {format_token_amount(total, config['decimals'])} {config['symbol']} "
        f"to {len(recipients)} users in {len(batches)} batch(es). "
        f"Skipped: {len(skipped)}. Each batch succeeds or fails together.")
    try:
        await execute_batches(service, db, key, run, sender['private_key_hex'], SUI_GAS_BUDGET)
    except Exception:
        logging.exception("Airdrop batch journal could not be updated for run %s", run_id)
        await update.message.reply_text(f"Airdrop stopped. Use /airdrop status {run_id} to reconcile saved results before retrying.")
        return
    await _report_airdrop(update.message, run)
...
async def setup_bot_commands(application):
    commands = [
        ...
        BotCommand("airdrop", "Batch rewards: /airdrop 10 10000 or 1-5:30k 6-10:15k"),
