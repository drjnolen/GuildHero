...
- **/setemoji** `<emoji|reset>` — Set the group's buybot emoji; use `reset` to restore 🔥 (admin only)
...
Use `/setemoji 🚀` to replace the buy-size flame row in this group's text announcements and media captions. The setting is saved across restarts and does not affect other groups, buyer badges, or the line icons. Send a single Unicode emoji (including flags, skin-tone variants, and compound emojis); Telegram Premium custom emoji artwork is not reproduced. `/setemoji` shows the current setting, and `/setemoji reset` (also `off` or `default`) restores 🔥. Buy-value scaling is unchanged; long emoji sequences are repeated fewer times with a count multiplier to keep the row within the default caption-size budget.
...
The bot registers `/settoken`, `/setbuybot`, `/setbuyimage`, `/setemoji`, `/setminbuy`, and the other supported commands with Telegram at startup so they appear in group command suggestions.
